...
export const DELETE_EXERCISE = async (req: Request, res: Response) => {
  try{
    if(req.query.id){
      ...
      if(!req.query.id){
      ...
      }else if(deletedExercise.deletedCount){
        ...
        throw new Error(`${deletedExercise.deletedCount} exercise has been matched with the id entered, please check the id entered.`);
      }
    }
