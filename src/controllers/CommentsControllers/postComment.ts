...
import CommentForAnswer from "../../notifications/executors/CommentForAnswer";
  ...
  try {
    ...
    if (owner && answer && content) {
      ...
      if (commentRepeatedOrNot) {
        throw new Error(
          "A comment with that content and owner has already exists."
        );
      } else {
        ...
        if (commentCreated) {
          ...
          CommentForAnswer(owner, answer);
        } else {
          throw new Error(
            "The comment hasn't been created, please check the inputs."
          );
        }
      }
    }
    ...
    res
      .status(400)
      .json(
        `An error has been ocurred in controller CREATE_COMMENT: ${err.message}`
      );
  }
};
